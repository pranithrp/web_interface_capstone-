...
def listen_for_events():
    ...
    console.print("✅ Blockchain connected — listening for security events...")
            # Create prediction DataFrame with only the feature columns
            pred_df = pd.DataFrame([sample])[FEATURE_COLUMNS]
            # Add trigger count after creating prediction DataFrame

            sample_df = pd.DataFrame([sample])
        for name, flt in filters.items():
            pred = int(rf_model.predict(pred_df)[0])
...
def run_detection(iterations=100, delay=1.0):
    ...
    for i in range(iterations):
        ...
        triggered = [f for f in FEATURE_COLUMNS if check_threshold(f, sample[f])]
            pred_df = pd.DataFrame([sample])[FEATURE_COLUMNS]  # Prediction with only feature columns
            sample["rule_trigger_count"] = len(triggered)      # Add trigger count after prediction
            sample_df = pd.DataFrame([sample])                 # Full sample for logging/display
            pred = int(rf_model.predict(pred_df)[0])
