...
async function testFedShieldIntegration() {
  ...
  try {
    ...
    try {
      const initResponse = await axios.post(`${BASE_URL}/init-database`);
      console.log('✅ Database init response:', initResponse.data);
    } catch (error) {
      console.log('⚠️ Database init failed:', error.response?.data || error.message);
    }
